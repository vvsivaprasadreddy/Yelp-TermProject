...
business_toronto = business[business.city == 'Toronto']

business_toronto.groupby('business_id').count()

business_toronto.set_index('business_id').index.get_duplicates()

data = business_toronto.merge(review, on = 'business_id')
...
data_bus_review_user[['business_id','name_x','user_id','name_y','stars_x'
                      ...
                      ,'useful_x','funny_x','cool_x','friends']].to_csv('toronto.csv')
#read toronto data
dt = pd.read_csv('toronto.csv')
